...
//   //伪随机，打乱vector
//   auto random_change_vector = [](std::vector<int> &a) {
...
//   }
// }
TEST_F(LSMTest, BigPersistence3) {
  //固定num的后一半会被删除
  int num = 500;
  std::vector<int> idx1, idx2;
  idx1.reserve(num / 2 + 1), idx2.reserve(num / 2 + 1);
  ...
  // Define a predicate function
  auto predicate = [num](const std::string &key) -> int {
    // Extract the number from the key
    // int key_num = std::stoi(key.substr(3));
    if (key < "key50") {
      return 1;
  ...
  auto res = lsm.lsm_iters_monotony_predicate(0, predicate);
  ASSERT_EQ(res.has_value(), true);
  auto [begin_iter, end_iter] = res.value();

  // int count = (num / 2) - 50 + 1;
  for (auto it = begin_iter; it != end_iter; ++it) {
    std::string key = it->first;
    std::string value = it->second;

    std::string kk = key.substr(3);
    // if (kk > "key999999" or kk < "key50") {
    std::cout << key << key.substr(3) << std::endl;
    // }
    // ASSERT_EQ(kk <= "key999999", true);
    // ASSERT_EQ(kk >= "key50", true);
